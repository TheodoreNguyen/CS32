...
bool StudentWorld::AllJewelsCollected()
{
	if (m_player->jewelsLeft() == 0)
		return true;
	return false;
}
		...
		for (int x = 0; x != VIEW_WIDTH; x++)
		{
			for (int y = 0; y != VIEW_HEIGHT; y++)
			{
				...
				case Level::exit:
					...
					m_player->setExit(m_actors[m_actors.size() - 1]);
					break;
				case Level::jewel:
					StudentWorld::m_actors.push_back(new Jewel(x, y, this));
					m_actors[m_actors.size() - 1]->GraphObject::setVisible(true);
					m_player->initializeJewel();
					break;
				default:
					break;
				}
...
int StudentWorld::move()
{
	...
	if (AllJewelsCollected())
		m_player->getExit()->setVisible(true);
